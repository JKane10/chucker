...
internal interface HttpTransactionDao {
    ...
    @Query(
        "SELECT id, requestDate, tookMs, protocol, method, host, path, scheme, responseCode, " +
            ...
            "graphQlOperationName, wasResponseMocked, shouldUseMock FROM transactions ORDER BY" +
            " requestDate DESC"
    )
    ...
    @Query(
        "SELECT id, requestDate, tookMs, protocol, method, host, path, scheme, responseCode, " +
            "requestPayloadSize, responsePayloadSize, error, graphQLDetected, " +
            "graphQlOperationName, wasResponseMocked, shouldUseMock FROM transactions WHERE " +
            "responseCode LIKE :codeQuery AND (path LIKE :pathQuery OR graphQlOperationName LIKE" +
            " :graphQlQuery) ORDER BY requestDate DESC"
    )
    ...
    @Query("SELECT * FROM transactions WHERE url = :url")
    fun getTransactionByUrl(url: String): List<HttpTransaction>
}
